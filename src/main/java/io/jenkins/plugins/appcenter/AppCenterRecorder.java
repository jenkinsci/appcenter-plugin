...
import io.jenkins.plugins.appcenter.task.AppCenterServiceFactory;
...
public final class AppCenterRecorder extends Recorder implements SimpleBuildStep {
    ...
    private boolean uploadToAppCenter(Run<?, ?> run, FilePath filePath, TaskListener taskListener) throws IOException, InterruptedException {
        ...
        try {
            final AppCenterServiceFactory appCenterServiceFactory = new AppCenterServiceFactory(getApiToken(), getOwnerName(), getAppName(), getPathToApp(), getBaseUrl());
